...
export default function Knockout() {
  ...
  async function loadAll() {
    setLoading(true)
    const [standingsRes, groupRes, koRes] = await Promise.all([
  ...
  async function generateQ2() {
    ...
    if (!q1Result) { alert('Qualifier 1 is tied — set a winner override on it first (Standings page equivalent for knockout, below).'); return }
          ...
          <table>
            ...
            <tbody>
              {standings.slice(0, 4).map(s => (
                <tr key={s.team_id}><td>{s.league_rank}</td><td><b>{s.team_name}</b></td><td>{s.cumulative_points}</td></tr>
              ))}
            </tbody>
          </table>
...
function KnockoutMatchupCard({ matchup, label, onChanged, isFinal }) {
  ...
  return (
    <div className="card">
      ...
      <p style={{ color: 'var(--muted)', fontWeight: 700 }}>
        Calculated: <b>{matchup.team_a?.name} {calcA}</b> — <b>{calcB} {matchup.team_b?.name}</b>.
