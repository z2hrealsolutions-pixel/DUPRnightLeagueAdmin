...
function KnockoutMatchupCard({ matchup, label, onChanged, isFinal }) {
  ...
  async function saveCourt() {
    if ((court || null) === (matchup.court || null)) return
    setSavingCourt(true)
    const { error } = await supabase.from('matchups').update({ court: court || null }).eq('id', matchup.id)
    if (!error) {
      // Same cascade as the Schedule page - the public view reads court
      // from each sub-match, not the parent matchup, so this must be
      // kept in sync or the change never actually shows up publicly.
      await supabase.from('sub_matches').update({ court: court || null }).eq('matchup_id', matchup.id)
    }
    setSavingCourt(false)
    if (error) { alert(error.message); return }
    onChanged()
    ...
    <div className="card">
      ...
      <div className="toolbar" style={{ marginTop: -8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <label style={{ marginBottom: 0 }}>Court</label>
          <input type="text" value={court} onChange={e => setCourt(e.target.value)} onBlur={saveCourt}
            placeholder="e.g. Court B" style={{ width: 110 }} disabled={savingCourt} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ marginTop: 14 }}>
        <label>
          {calcA === calcB ? `Tied at ${calcA} points each. Set the winner manually` : 'Set or override the winner manually'}
        </label>
