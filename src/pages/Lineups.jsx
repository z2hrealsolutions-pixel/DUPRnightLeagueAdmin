...
import { MATCH_TYPES, pointsForType, STAGE_LABELS } from '../constants'
        ...
        <select value={selectedId} onChange={e => setSelectedId(e.target.value)}>
          ...
          {matchups.map(m => (
            <option key={m.id} value={m.id}>
              {stageLabel(m.stage)} - {m.team_a?.name} vs {m.team_b?.name} {m.scheduled_date ? `(${m.scheduled_date})` : ''} ({m.status})
            </option>
          ))}
        </select>
      </div>
...
function LineupEditor({ matchup }) {
  ...
  return (
    <div className="card">
      <h3>{stageLabel(matchup.stage)}: {matchup.team_a?.name} vs {matchup.team_b?.name}</h3>
