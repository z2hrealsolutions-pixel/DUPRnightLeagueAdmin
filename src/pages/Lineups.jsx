    ...
    try {
      ...
      if (subMatchRows.length > 0) {
        const { error: subErr } = await supabase
          .from('sub_matches')
          .upsert(subMatchRows, { onConflict: 'matchup_id,match_type,slot_number', ignoreDuplicates: true })
...
function TeamSlotSelect({ teamName, roster, type, value, onChange }) {
  return (
    <div style={{ flex: 1, display: 'flex', gap: 6 }}>
      <select value={value?.p1 || ''} onChange={e => onChange('p1', e.target.value)} style={{ flex: 1 }}>
        <option value="">{teamName} — select player</option>
        {roster.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
      </select>
      {type.isDoubles && (
        <select value={value?.p2 || ''} onChange={e => onChange('p2', e.target.value)} style={{ flex: 1 }}>
          <option value="">+ partner</option>
          {roster.filter(p => p.id !== value?.p1).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
      )}
    </div>
  )
