...
function MatchupRow({ matchup, onSave }) {
  ...
  function save() {
    const fields = { scheduled_date: date || null, scheduled_time: time || null }
    // Only include court when it's genuinely different from what's saved -
    // this field triggers a cascade to every sub-match, so it must never
    // fire just because some other field on this row lost focus.
    if ((court || null) !== (matchup.court || null)) {
      fields.court = court || null
    }
    onSave(matchup.id, fields)
  }
