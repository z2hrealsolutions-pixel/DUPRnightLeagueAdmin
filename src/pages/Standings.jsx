    ...
    <div>
      ...
      <div className="card">
        ...
        {loading ? <div className="loading-note">Loading…</div> : (
          <table>
            <thead>
              <tr><th>Rank</th><th>Team</th><th>Face-offs Played</th><th>Cumulative Points</th></tr>
            </thead>
            <tbody>
              {standings.map(s => (
                <tr key={s.team_id}>
    ...
    <div className="card">
      ...
      <div style={{ marginTop: 20, borderTop: '2px solid var(--surface-2)', paddingTop: 16 }}>
        ...
        <p style={{ color: 'var(--muted)', fontWeight: 600, fontSize: '.85rem', marginTop: 0 }}>
          Calculated from sub-matches: <b>{matchup.team_a?.name} {calculatedA}</b> — <b>{calculatedB} {matchup.team_b?.name}</b>.
