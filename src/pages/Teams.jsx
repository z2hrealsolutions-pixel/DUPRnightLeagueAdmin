...
function TeamCard({ team, expanded, onToggle, onDeleteTeam, onChanged }) {
  ...
  return (
    <div className="card">
      ...
      {expanded && <RosterEditor team={team} players={players} onChanged={onChanged} />}
    </div>
  )
}
        ...
        <tbody>
          {players.map(p => (
            <tr key={p.id}>
              <td>{p.name}</td>
              <td>{p.gender}</td>
              <td>{p.dupr_id || (p.dupr_email ? `email: ${p.dupr_email}` : '—')}</td>
              <td>{p.is_45_plus ? 'Yes' : ''}</td>
              <td><button className="btn danger small" onClick={() => deletePlayer(p.id)}>Remove</button></td>
            </tr>
          ))}
          {players.length === 0 && (
